...
class VQA(data.Dataset):
    ...
    def _encode_question(self, question):
        ...
        # Encode text
        # bvec = torch.tensor([self.bert_tokenizer.encode(ques) for ques in question])
        str = ''
        for s in question:
            str += s
            str += ' '
        try:
            bvec = torch.tensor(self.bert_tokenizer.encode(str)).long()
        except:
            self.bert_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
            bvec = torch.tensor(self.bert_tokenizer.encode(str)).long()

        for i, token in enumerate(bvec):
            if i >= self.max_question_length:
                break
            vec[i] = token
        return vec, min(len(question), self.max_question_length)
